(() => {
  ...
  const caseModalData = {
    ...
    "gestiones-catastrales": {
      ...
      steps: [
        ...
        "Revisión de parcelas, límites, colindancias y documentación.",
        "Delimitación técnica de la propiedad con soporte GNSS + CAD + GIS."
      ]
    },
  ...
  function openCaseModal(tile) {
    ...
    if (img) {
      ...
      const imageWidth = parseInt(img.getAttribute('width'), 10) || img.naturalWidth || 1;
      const imageHeight = parseInt(img.getAttribute('height'), 10) || img.naturalHeight || 1;
      caseModal.style.setProperty('--case-modal-image-ratio', `${imageWidth} / ${imageHeight}`);
      ...
      caseModalImg.onload = () => {
        if (caseModalImg.naturalWidth && caseModalImg.naturalHeight) {
          caseModal.style.setProperty('--case-modal-image-ratio', `${caseModalImg.naturalWidth} / ${caseModalImg.naturalHeight}`);
        }
      };
    }
...
window.extraCaseData.catastro = {
  ...
  text: "Servicio de apoyo para consulta catastral, certificación, revisión parcelaria y delimitación técnica de la propiedad inmobiliaria.",
  bullets: ["Consulta y certificación catastral", "Revisión parcelaria y colindancias", "Delimitación técnica con GNSS, CAD y GIS"]
};
