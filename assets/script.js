(function () {
  ...
  // Delegación extra: garantiza los saltos internos incluso si el navegador o la caché ignoran el listener directo.
  document.addEventListener('click', function (event) {
    var link = event.target && event.target.closest ? event.target.closest('a[href^="#"]') : null;
    if (!link) return;
    var href = link.getAttribute('href') || '';
    if (href === '#contacto') scrollToSection('contacto', event);
    else if (href === '#top') scrollToSection('top', event);
  }, true);
