(() => {
  ...
  const mobileLite = matchMedia('(max-width: 760px), (pointer: coarse)').matches;
  ...
  if (glow && !mobileLite && !reducedMotion) {
  ...
  if (!mobileLite && !reducedMotion) $$('.tilt').forEach((card) => {
  ...
  if (!mobileLite && !reducedMotion) $$('.magnetic').forEach((btn) => {
  ...
  if (canvas && ctx && !reduced && !mobileLite) {
    let w = 0, h = 0, dpr = 1;
    const points = [];
    const total = Math.min(110, Math.floor((innerWidth * innerHeight) / 12000));
    function resize() {
  ...
  const sparkLayerOK = !mobileLite && !reducedMotion;
  if (sparkLayerOK) {
    for (let i = 0; i < 24; i++) {
